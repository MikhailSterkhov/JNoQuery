...
public class SQLRequestInsert extends SQLRequestQuery implements RequestInsert {

    private static final String QUERY = "INSERT {ignored} INTO `{content}` ({keys}) VALUES ({values}) {update_keys}";

    SQLRequestSessionAppender<ValueDataField, RequestInsert> appender = new SQLRequestSessionAppender<>(this);

    @NonFinal
    boolean ignored, updateOnDuplicateKeys;

    public SQLRequestInsert(SQLRequest request) {
        super(request);
    }
